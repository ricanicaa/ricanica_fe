...
export const CreateLetter = ({ searchQuery }) => {
  ...
  useEffect(() => {
    ...
    const matchedName = names.find((item) => item.english_name === searchQuery);
      ...
      <form className="LetterForm" onSubmit={handleSubmit}>
        ...
        <div className="FormField">
          ...
          {!isSenderValid && sender.length === 0 && (
            <div className="helper-text">보내는 사람을 입력해주세요.</div>
          )}
        </div>
        <div
          style={{
        ...
        >
          <button
            type="submit"
            className={`SubmitButton ${isValid ? "active" : "disabled"}`}
            disabled={!isValid}
            style={{
              display: "flex",
              flexDirection: "row",
